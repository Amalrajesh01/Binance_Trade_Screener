...
def send_telegram_message(text: str):
    """Send a message to your Telegram bot with retries"""
    ...
    for attempt in range(3):
        ...
        time.sleep(2)
    logger.error("Failed to send Telegram message after 3 attempts: %s", text)
    return None
...
def run_screener():
    ...
    send_telegram_message(f"🚀 Starting Binance FVG Screener - {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    
    symbols = get_futures_symbols()
    if not symbols:
        logger.error("No symbols fetched, aborting screener")
        send_telegram_message(f"❌ No symbols fetched, screener aborted - {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
        return
...
@app.route("/health")
def health_check():
    status = {"status": "running", "scheduler": scheduler.running, "timestamp": time.strftime('%Y-%m-%d %H:%M:%S %Z')}
...
def ping_self():
    ...
    for attempt in range(3):
        try:
            ...
            logger.error("Ping failed (attempt %d): %s", attempt + 1, str(e))
        time.sleep(5)  # Longer delay between retries
    # Fallback: Ping an external service to wake the app
    fallback_url = "https://httpstat.us/200"
    try:
        requests.get(fallback_url, timeout=10)
        msg = f"❌ Ping to app failed, used fallback - {time.strftime('%Y-%m-%d %H:%M:%S %Z')}"
        logger.warning(msg)
        send_telegram_message(msg)
    except Exception as e:
...
def scheduled_job():
    ...
    try:
        ...
        send_telegram_message(f"❌ Scheduled job failed: {str(e)} - {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        send_telegram_message(f"❌ Initial screener run failed: {str(e)} - {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    ...
    scheduler.add_job(ping_self, "interval", minutes=5)  # Increased frequency to 5 minutes
    
    try:
        scheduler.start()
        logger.info("Scheduler started with cron jobs at 01:32, 05:32, 09:32, 13:32, 17:32, 21:32 IST")
        send_telegram_message(f"✅ Scheduler started - {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    except Exception as e:
        logger.error("Scheduler failed to start: %s", str(e))
        send_telegram_message(f"❌ Scheduler failed to start: {str(e)} - {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
